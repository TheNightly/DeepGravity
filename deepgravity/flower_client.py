...
parser.add_argument('--client-idx', type=int, default=0, metavar='N',
                    help='index of client to for dataset partitioning (default: 0)')
parser.add_argument('--client-size', type=int, default=2, metavar='N',
                    help='number of clients for dataset partitioning (default: 2)')
...
parser.add_argument('--dataset', default='new_york')
    ...
    for batch_idx, data_temp in enumerate(train_loader):
        ...
        if batch_idx % args.log_interval == 0:
            if batch_idx * len(b_data) == len(train_loader) - 1:
                print('Train Epoch: {} [{}/{} \tLoss: {:.6f}'.format(epochs, batch_idx * len(b_data), len(train_loader), loss.item()/args.batch_size))
...
def evaluate(model, test_loader):
    ...
    return cpc_df.cpc.mean(), cpc_df.cpc.std()
    # flow_list = []
...
client_size = len(train_data) // args.client_size
start_idx = client_idx * client_size
end_idx = (client_idx + 1) * client_size
my_data = train_data[start_idx:end_idx]
print(f"Using indices {start_idx}:{end_idx}")
train_dataset = dgd.FlowDataset(my_data, **train_dataset_args)
...
if args.mode == 'train':
    ...
    class FlowerClient(fl.client.NumPyClient):
        ...
        def evaluate(self, parameters, config):
            ...
            cpc_avg, cpc_dev = evaluate(model, test_loader)
            return float(loss), len(test_loader.dataset), {"cpc": float(cpc_avg), "std_dev": float(cpc_dev)}
